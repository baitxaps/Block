...
/*
 ...
 变量
 2.另外,__block变量的__Block_byref_val_0结构体并不在Block用__main_block_impl_0结构体中，这样做是为了在多
 个Block中使用__block变量：
 __block int val = 6;
 void (^blk0)(void) = ^{val = 0;};
 void (^blk1)(void) = ^{val = 1;};
 Block类型变量blk0 、blk1访问__block言火日王val：
...
/*
 ...
 在函数内而不在记述广域变量的地方使用Block语法时，只要Block不截获自动变量，就可以将Block用结构体实例设置在
 程序的数据区域：
...
/*
 ...
 在任何一个Block从栈复制到堆时，__block变量也会一并从栈复制到堆并被该Block所持有，当剩下的Block从栈复制
 到堆时，被复制的Block持有__block变量，并增加__block变量的引用计数
 3>如果配置在堆上的Block被废弃，那么它所使用的__block变量也就被释放
 
 4>不管__block变量配置在栈上还是在堆上，都能够正确地访问该变量（同一个__block变量）（通过Block的复制，__block变量也从栈复制到堆，此
 时可同时访问栈上的__block变量和堆上__block变量）
